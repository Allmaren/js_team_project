...
function getFetchTrending(page = 1) {
  let currentTrendPage = localStorage.getItem("currentTrendPage")? localStorage.getItem("currentTrendPage") : 1;

  return fetch(
    `${BASE_URL}${TRENDING}?api_key=${API_KEY}&language=en-US&page=${isFirstFetch?currentTrendPage:page}&include_adult=false`
  )
